...
handler
  ...
  .patch(<%= h.changeCase.camelCase(name) %>Routes.update, async (req: any, res) => {
    const updated<%= h.changeCase.pascalCase(name) %> = await update<%= h.changeCase.pascalCase(name) %>({ <%= h.changeCase.camelCase(name) %>Id: req.body.<%= h.changeCase.camelCase(name) %>Id, <%= h.changeCase.camelCase(name) %>Input: req.body.attrs});
